...
Although `Team` is a top-level object, it can also be used in relation to other objects. For instance, it is part of the [`Match`](@site/docs/api-guide/objects-business/match/match.md) object, which contains `homeTeam` and `awayTeam` objects expressing which team acts as the home team and which as the away team in a specific match.
