...
Each element in the response array corresponds to exactly one period. It always includes a `timeframe` object that identifies which period the metrics belong to — see the [Time Frame](/api-guide/objects-common/time-frame) chapter for a full description of its attributes (`phase`, `period`, and the various time attributes).
